...
/*   Updated: 2017/11/10 14:34:04 by ccazuc           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pixel_put(t_env *env, int x, int y, unsigned int color)
{
	...
	{
		printf("x: %d, y: %d\n", x, y);
		env->mlx_img_data[y * WINDOW_WIDTH * 3 + 3 * x] = b;
		env->mlx_img_data[y * WINDOW_WIDTH * 3 + 3 * x + 1] = g;
		env->mlx_img_data[y * WINDOW_WIDTH * 3 + 3 * x + 2] = r;
	}
